...
export async function select_clinics (req, res) {
    ...
    const response_select_clinics = await models.select_clinics()

    if(!response_select_clinics.status){
        return res.status(400).json({status: "error", message: response_select_clinics.message})
    }

    return res.status(200).json({status: "success", clinics: response_select_clinics.data})
}